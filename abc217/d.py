...
# その場所の両隣間の差を出力すれば良い
# pythonの標準ライブラリには2分探索で挿入できるデータ構造がないので、無理
# sortedcontainers.SortedList(外部ライブラリ)を使えばいける
...
L, Q = [int(s) for s in input().split()]
wood = SortedList()
for _ in range(Q):
    c, x = [int(s) for s in input().split()]
    if c == 1:
        wood.add(x)
    elif c == 2:
        idx = bisect_left(wood, x)
        #print(wood, idx)
        if idx == 0:
            l = 0
        else:
